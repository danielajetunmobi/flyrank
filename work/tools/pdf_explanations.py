...
("w07_action_playbook", 2):
   "Setup, and deliberately not a re-derivation. ML-10 rebuilds the ML-08 cohort, frozen gate and "
   "four-feature model from the same code rather than re-fitting anything, because a playbook that "
   "quietly changed the model would be recommending something no earlier notebook validated. The "
   "printed line exists to be compared against ML-08's own output -- 45,095 pages, 30 clients, "
   "decline rate 0.5131 -- so a silent drift in the upstream data shows up here as a mismatch "
   "rather than as a subtly different playbook.",
 ("w07_action_playbook", 4):
   "queue_metrics returns pooled and macro precision from one pass. Pooled sums hits over picks "
   "across all clients; macro averages each client's own rate. FlyRank asked for macro as the "
   "headline, and the two differ by -0.0324 at K=100 because the average client is not the average "
   "page -- a handful of large clients dominate the pooled figure. The K sweep then runs the model "
   "and a random baseline at every budget from 10 to the whole pool. K='all' is the control: a "
   "queue containing every page cannot beat random, so a non-zero lift there would prove the "
   "harness wrong rather than the model good.",
 ("w07_action_playbook", 6):
   "Builds the deliverable. The model is fitted on the entire evaluation pool because a queue "
   "shipped on the decision date has no held-out set -- every page needs a score. Reason codes are "
   "generated by ranking each page's standardised feature values against the client's own median, "
   "so they are literally a readout of what moved the score, not a post-hoc story. That is what "
   "makes the printed reason distribution worth reading: it shows the queue's composition is "
   "dominated by one feature, which is a fact about the model rather than about the pages.",
 ("w07_action_playbook", 9):
   "The outcome-window sweep FlyRank asked for. One SQL pass cuts the future window at 7, 14, 30, "
   "60 and 90 days using FILTER clauses, because the prior-side aggregation is identical for every "
   "horizon and only the future sums need re-cutting -- five separate queries would have taken five "
   "times as long for the same answer. Each window relabels the SAME pages with the SAME features "
   "and refits, so any change in lift is the horizon and nothing else. The second half splits the "
   "pool on peak_ratio < 1 to test whether the model can only rank pages that have already started "
   "falling, which section 1 had assumed from queue composition alone.",
 ("w07_action_playbook", 12):
   "Three no-go tests. The decile analysis bins out-of-fold predictions and reports the actual "
   "decline rate in each, which answers whether the list degrades gracefully or falls off a cliff, "
   "and names the first decile that is no better than the pool base rate -- the honest place to "
   "stop working a queue. The volume strata ask whether small pages are rankable at all, reported "
   "as share of ceiling because base rates differ by quartile and raw lift would mislead. The dead- "
   "page check compares how often pages reaching zero appear in the top 100 against the pool, since "
   "a page at zero is the largest possible decline and will sort to the top whether or not a "
   "refresh is the right response to it.",
 ("w07_action_playbook", 15):
   "The weekly walk-forward, rebuilt at the cadence FlyRank confirmed. ML-09 spaced its decision "
   "points monthly and flagged that as answering the cadence the project had assumed; the warehouse "
   "allows fourteen weekly points between 2026-03-01 and 2026-06-01, giving thirteen forward steps "
   "against three. Each step trains on one week and scores the next, never backwards. The final "
   "block is the point of the section: it compares how raw lift and share-of-ceiling each correlate "
   "with the test week's base rate, because those two correlations decide which of them a "
   "monitoring dashboard should plot.",
 ("w07_action_playbook", 17):
   "The confound test, and the most important cell in the notebook. Before setting an alert on a "
   "drifting base rate it asks what the base rate is measuring, by computing a panel-level quantity "
   "-- total impressions divided by page-days across every client -- in the same baseline and "
   "future windows the target uses. This number cannot know anything about an individual page, so "
   "if it tracks the decline rate then the decline rate is not a fact about content. It correlates "
   "-0.9784, R-squared 0.9573. That retires the attribution in ML-09 section 8 and the capstone, "
   "which read the same movement as a property of the client pool.",
 ("w07_action_playbook", 20):
   "The export, with two assertions that are the actual content of the cell. Guard 1 checks no "
   "outcome column reaches the file: queue carries `declined`, the column selection drops it, and "
   "the assertion proves the drop rather than trusting it -- a queue containing the answer is a "
   "mark sheet, not a queue. Guard 2 checks nothing identifying leaves the repo. Both guards were "
   "wrong when first written and both were fixed by looking at the data rather than relaxing the "
   "test: the hash pattern demanded bare hex when the real format is client_ plus sixteen hex, and "
   "the text-column test asked for dtype == object, which matches nothing under pandas 3 and "
   "reported PASS having inspected zero columns. It now fails when it cannot find its subject.",
}
